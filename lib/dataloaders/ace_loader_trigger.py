...
class ACE_Dataset_Trigger(Dataset):
    ...
    def _load_data_ent(self, data_path, wdict, edict, ydict, max_len):
        ...
        for line in open(data_path):
            ...
            words, ents, labels = self._sent2array_ent((wds, ents, event), wdict, edict, ydict, max_len)
            sen.append(words)
            ent.append(ents)
            y.append(labels)
    ...
    def _sent2array_ent(self, sent, wdict, edict, ydict, max_len):
        ...
        events = list(map(lambda x: ydict.get(x.lower(), 'negative'), sent[2]))
        MAX_SEN_LEN = max_len
        ...
        if len(words) > MAX_SEN_LEN:
            ...
            events = events[:MAX_SEN_LEN]
        # labels = [ydict.get(x.lower(), 'negative') for x in sent[2]]
        return words, ents, events
    ...
    def __getitem__(self, item):
        ...
        events = self.data_loaded[2][item]
        token_len = tokens.shape[0]
        # event = self.label_hot(event)
        return tokens, entity_tag, events, token_len
...
Custom_trigger_loader = partial(DataLoader, collate_fn=collate_fn, pin_memory=True)